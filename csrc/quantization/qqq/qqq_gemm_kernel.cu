...
__device__ inline FragB dequant_per_group(int q, FragS_GROUP& frag_s, int i) {
  ...
  *reinterpret_cast<half2*>(&t0) = __hsub2(*reinterpret_cast<half2*>(&t0),
...
template <const int threads,          // number of threads in a threadblock
          ...
          >
__global__ void
Marlin(const int4* __restrict__ A,    // int8 input matrix of shape mxk 
       const int4* __restrict__ B,    // 4bit quantized weight matrix of shape kxn 
             int4* __restrict__ C,    // int32 global_reduce buffer of shape (max_par*16*4)xn,
                                      // as int8 tensor core's output is int32 dtype
       ...
       const int4* __restrict__ s3,   // fp16 weight per-group quantization scales of shape (k/groupsize)xn,
                                      // when group_blocks=-1, it should be nullptr
...
torch::Tensor qqq_gemm(torch::Tensor& a, torch::Tensor& b_q_weight,
  ...
  TORCH_CHECK(s2.numel() == size_n,
              "Shape mismatch: s2.numel() = " + str(s2.numel()) +
                  ", size_n = " + str(size_n));
  TORCH_CHECK(b_q_weight.size(1) % vllm::qqq::tile_size == 0,
  ...
  TORCH_CHECK(
      size_n % vllm::qqq::min_thread_n == 0,
      "size_n = " + str(size_n) +
          ", is not divisible by min_thread_n = " + str(vllm::qqq::min_thread_n));
  int min_workspace_size = (size_n / vllm::qqq::min_thread_n) * vllm::qqq::max_par;
  ...
  vllm::qqq::qqq_cuda(a.data_ptr(), b_q_weight.data_ptr(), c.data_ptr(),
                      d.data_ptr(), s1.data_ptr(), s2.data_ptr(), s3.data_ptr(),
                      size_m, size_n, size_k, workspace.data_ptr(), groupsize, dev,
                      at::cuda::getCurrentCUDAStream(dev), thread_k, thread_n,
                      sms, vllm::qqq::max_par);
